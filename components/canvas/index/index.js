const main = {
  ...
  drawBlock({
    ...
    if (backgroundColor) {
      ...
      if (borderRadius > 0) {
        ...
        if(linearGradient){
          let x0 = this.toPx(linearGradient.x0) || 0;
          let y0 = this.toPx(linearGradient.y0) || 0;
          let x1 = this.toPx(linearGradient.x1) || 0;
          let y1 = this.toPx(linearGradient.y1) || 0;
          let colorStop = linearGradient.colorStop || [];
          // Create linear gradient
          const grd = ctx.createLinearGradient(x0, y0, x1, y1);
          if(colorStop.length > 0){
            colorStop.forEach((item, index) => {
  ...
  drawImage(data) {
    ...
    if (borderRadius > 0) {
      ...
      this.ctx.drawImage(imgPath, this.toPx(sx), this.toPx(sy), this.toPx(sw), this.toPx(sh), this.toPx(x), this.toPx(y), this.toPx(w), this.toPx(h));
      // console.log('imgPath: ', imgPath,sx,sy,sw,sh);
      // 0 247.8260869565217 652.1739130434783 373.9130434782609
    }
    this.ctx.restore();
  },
  /**
...
const handle = {
  ...
  _drawSingleText({
    ...
    textArr.forEach((item, index) => {
      item && this.ctx.fillText(item, this.toPx(x), this.toPx(y + (lineHeight || fontSize) * index));
    })
    ...
    return new Promise((resolve, reject) => {
      ...
      this._downImage(imageUrl, index)
        ...
        .then(({
          ...
          /* var y = sy;
          if(image.sx === 0 || image.sx){
            sx = image.sx;
            sy = image.sy;
            if (width / height <= setWidth / setHeight) {
              y = (height - ((width / setWidth) * setHeight)) / 2;
            } else {
              y = 0;
            }
          }else{
